...
impl TestRayMarcher {
    pub fn new(wgpu_backend: &WGPUBackend, logic: &Logic) -> Self {
        ...
        let camera_inverted_projection_data = logic.camera.get_inverted_projection_matrix(wgpu_backend.config.0.width as f32 / wgpu_backend.config.0.height as f32);
        ...
        let surface_configuration_data = [wgpu_backend.config.0.width as f32 / wgpu_backend.config.1, wgpu_backend.config.0.height as f32 / wgpu_backend.config.1];
